...
void upper_points(t_point *, int, t_point, t_point, t_point *, int *);
...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity)
{
	if (!num_point){
		...
		if (*num_line >= *capacity){
			lines = realloc(lines, ((*capacity) + 10) * sizeof(t_line));
			(*capacity) += 10;
		}
		return lines;
	}
	...
	upper_points( points, num_point, p1, pmax, s1, &n1);
	lines = upper_hull( s1, n1, p1, pmax, lines, num_line, capacity);
	upper_points( points, num_point, pmax, pn, s2, &n2);
	lines = upper_hull( s2, n2, pmax, pn, lines, num_line, capacity);
...
void upper_points( t_point *points, int num_point, t_point from, t_point to, t_point *s, int *n)
{
	*n = 0;
	int a = to.y - from.y;
	int b = from.x - to.x;
